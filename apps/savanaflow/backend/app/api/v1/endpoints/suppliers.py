"""Suppliers endpoints — SavanaFlow."""
...
from app.schemas.schemas import Paginated, SupplierCreate, SupplierOut
...
async def update_supplier(supplier_id: int, data: SupplierCreate, db: AsyncSession = Depends(get_db), _: User = Depends(get_current_user)):
...
async def delete_supplier(supplier_id: int, db: AsyncSession = Depends(get_db), _: User = Depends(get_current_user)):
    s = (await db.execute(select(Supplier).where(Supplier.id == supplier_id))).scalar_one_or_none()
    if not s:
        raise HTTPException(404, "Supplier not found")
    s.is_active = False
